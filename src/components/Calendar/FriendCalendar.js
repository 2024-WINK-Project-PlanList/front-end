...
const FriendCalendar = ({ userId, readOnly = true }) => {
  ...
  const fetchFriendCalendar = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/calendar/${userId}`,
        {
          headers: {
      ...
      console.log('친구 캘린더 정보:', response.data);
      ...
      console.error('친구 캘린더 데이터 가져오기 오류:', error);
      setPlans([]);
    }
  };
      ...
      >
        ...
        {Array.isArray(selectedDates) && selectedDates.length > 0 && (
          <CalendarPlan
            ...
            plans={plans}
            setPlans={setPlans}
            calendarId={userId} // 친구의 calendarId 사용
          />
        )}
      </div>
