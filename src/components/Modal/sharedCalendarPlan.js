...
import SharedCalendarBottomSheet from './sharedCalendarBottomSheet';
import myProfileImage from '../../assets/mainpage/profile.svg'; // 기본 프로필 이미지

const SharedCalendarPlan = ({
  isOpen,
  onClose,
  selectedDates,
  plans,
          ...
          <div className="modal-header flex flex-col justify-between items-start p-4">
            ...
            <div className="flex flex-col mt-4 space-y-2 w-full">
              ...
              {filteredPlans.map((plan, index) => {
                // scheduleMemberList가 정의되어 있는지 확인하고 기본값 설정
                const displayedMembers = (plan.scheduleMemberList || []).slice(
                  0,
                  3,
                );
...
export default SharedCalendarPlan;
