...
const SharedCalendarBottomSheet = ({
  isOpen,
  onClose,
  selectedDates = [],
  plan,
    ...
    try {
      ...
      if (plan && plan.id) {
        response = await axios.patch(
          `${process.env.REACT_APP_BACKEND_URL}/shared-schedule/${plan.id}`,
          newPlan,
          {
            headers: {
        ...
        response = await axios.post(
          `${process.env.REACT_APP_BACKEND_URL}/shared-schedule`,
          newPlan,
          {
            headers: {
              Authorization: `Bearer ${localStorage.getItem('token')}`,
  ...
  const fetchUpdatedPlans = async () => {
    try {
      const calendarResponse = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/shared-calendar`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
...
export default SharedCalendarBottomSheet;
